...
productsRouter.post('/', async (req: Request, res: Response) => {
  try {
    const { title, description, price, image_url, category_id, seller_id, status } = req.body as ProductCreate
    ...
    const result = await query(
      `INSERT INTO products (title, description, price, image_url, category_id, seller_id, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`,
      [title, description || '', price, image_url || null, category_id || null, seller_id, status || 'draft']
    )
