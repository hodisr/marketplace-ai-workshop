...
- `path/to/another/file.vue` - Brief description (e.g., Page component for this feature).
- `path/to/another/file.test.ts` - Unit tests for `another/file.vue`.

### Notes

- Unit tests should typically be placed alongside the code files they are testing (e.g., `MyComponent.vue` and `MyComponent.test.ts` in the same directory).
- Use `npx vitest run [optional/path/to/test/file]` to run tests (both frontend and backend).
